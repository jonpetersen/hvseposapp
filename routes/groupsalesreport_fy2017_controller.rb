class GroupsalesreportfinyrController < Sinatra::Base
  ...
  get '/' do
    ...
    @archivesalesbygroup = Archivesale.joins(:stock =>{:depart => :group}).where("type = ? AND MONTH(date) > ? AND YEAR(date) = ?","P",@repmonth,@finyr).group("departs.group")
    ...
    class Groupsales
      def self.totalqty(group)
        if group != "all"
          group_archivesales = Archivesale.joins(:stock =>{:depart => :group}).where("departs.group = ? AND type = ? AND MONTH(date) > ? AND YEAR(date) = ?",group,"P",@repmonth,@finyr).group("archivesales.plu")
          ...
          group_archivesales = Archivesale.joins(:stock =>{:depart => :group}).where("type = ? AND MONTH(date) > ? AND YEAR(date) = ?","P",@repmonth,@finyr).group("archivesales.plu")
      ...
      def self.salesbydate(group)
	    if group != "all"
          ...
          group_sales_values_archive = Archivesale.joins(:stock =>{:depart => :group}).where("departs.group = ? AND type = ? AND MONTH(date) > ? AND YEAR(date) = ?",group,"P",@repmonth,@finyr).group(:date).sum(:totalprice)
          ...
          group_sales_values_archive = Archivesale.joins(:stock =>{:depart => :group}).where("type = ? AND MONTH(date) > ? AND YEAR(date) = ?","P",@repmonth,@finyr).group(:date).sum(:totalprice)
          all_sales_values = group_sales_values_archive.merge(group_sales_values)
        end    
	  end
	  def self.salesbymonth(group)
	    if group != "all"
	      group_sales_values = Allsale.joins(:stock =>{:depart => :group}).where("departs.group = ? AND type = ?",group,"P").group_by_month(:created_at).sum(:totalprice)
          group_sales_values_archive = Archivesale.joins(:stock =>{:depart => :group}).where("departs.group = ? AND type = ? AND MONTH(date) > ? AND YEAR(date) = ?",group,"P",@repmonth,@finyr).group_by_month(:created_at).sum(:totalprice)
          all_sales_values = group_sales_values_archive.merge(group_sales_values)
        else
          group_sales_values = Allsale.joins(:stock =>{:depart => :group}).where("type = ? AND MONTH(created_at) = ?","P",Date.today.month).group_by_month(:created_at).sum(:totalprice)
          group_sales_values_archive = Archivesale.joins(:stock =>{:depart => :group}).where("type = ? AND MONTH(date) > ? AND YEAR(date) = ?","P",@repmonth,@finyr).group_by_month(:created_at).sum(:totalprice)
          all_sales_values = group_sales_values_archive.merge(group_sales_values)
        end    
	  end
    end
    ...
    @sales_values_archive = Archivesale.where("type = ? AND MONTH(date) > ? AND YEAR(date) = ?","P",@repmonth,@finyr).group(:date).sum(:totalprice)
    
    @sales_values_monthly = Allsale.where("type = ? AND MONTH(created_at) = ?","P",Date.today.month).group_by_month(:created_at).sum(:totalprice)
    @sales_values_archive_monthly = Archivesale.where("type = ? AND MONTH(date) > ? AND YEAR(date) = ?","P",@repmonth,@finyr).group_by_month(:created_at).sum(:totalprice)
    ...
    if @group != "all"
      ...
      @groupsalesseries_monthly = Groupsales.salesbymonth(@group)
    else
      @groupsalesseries = @all_sales_values
      @groupsalesseries_monthly = @all_sales_values_monthly 
    end
